...
public:
    ...
    Collectable() : ring("ring.png", 0, 0, 1.25f, 1.25f) {}
};
...
public:
    ...
    Button(string str, vector2f position, Color color = Color::Blue, int characterSize = 30)
    {
        ...
        rect.setSize({text.getGlobalBounds().width + 20, 50}); // Add some padding
...
public:
    ...
    // Load Image -> Set Sprite -> SetScale -> SetPosition
    EnemyFactory(string path = "fireBall.png", vector2f position = {-1000, -1000}, vector2f scale = {0, 0})
    {
...
public:
    ...
    enemies(
        ...
        : EnemyFactory(path, {startX, startY}, {scaleX, scaleY}),
          hp(100), isAlive(true),
...
public:
    ...
    Flyer(
        ...
        int frameHeight = 50,
        float frameDuration = 0.15f)
        : enemies(path, startX, startY, scaleX, scaleY,
...
public:
    ...
    // Constructor
    scoreBoard(int pos, const string &playerName, int playerScore)
...
void fileRead(scoreBoard *&ranking, int &totalRanks);
...
public:
    ...
    void animateMotoBug(float &elapsedTime, enemies &e)
    {
    ...
    void animateCrabMeat(float &elapsedTime, enemies &e)
    {
        e.elapsed += elapsedTime;
    ...
    void animateBeeBot(float &elapsedTime, enemies &e)
    {
        e.elapsed += elapsedTime;
    ...
    void animateBatBrain(float &elapsedTime, enemies &e)
    {
        e.elapsed += elapsedTime;
    ...
    void spawnEnemies(char screenLayout[][200], int rows, int cols,
                      Crawlers testEnemy1[], int e1, int &idx1,
                      Crawlers testEnemy2[], int e2, int &idx2,
                      enemies testEnemy3[], int e3, int &idx3,
                      Flyer testEnemy4[], int e4, int &idx4)
    {
        for (int i = 0; i < rows; i++)
        {
    ...
    void showTimeOverScreen(RenderWindow &window, Sonic &sonic, float duration)
    {
        ...
        while (window.isOpen() && clock.getElapsedTime().asSeconds() < duration)
        {
            ...
            appendToFile("leaderBoard.txt", playerName, sonic.getRingsCollected(), ranks); // Append username with initial score 0
    ...
    void showLevelUpScreen(RenderWindow &window, Sonic &sonic, float duration)
    {
        Clock clock;
    ...
    void levelOne(RenderWindow &window, Event &event)
    {
        ...
        const int rows = 12;
        const int cols = 200;
        // enemies testEnemy1("CrabMeat.png", 270, 101, 1.45f, 1.3f, 8, 54.f, 41);
        // enemies testEnemy2("motoBug.png", 101, 100, 1.12f, 1.12f, 4, 43.5f, 41);
        // enemies testEnemy3("beeBot.png", 101, 400, 2.f, 2.f, 5, 34.f, 20);
        // enemies testEnemy4("batBrain.png", 551, 100, 1.5f, 1.5f, 5, 65.f, 40);
        ...
        Crawlers *testEnemy1 = new Crawlers[e1];
        Crawlers *testEnemy2 = new Crawlers[e2];
        enemies *testEnemy3 = new enemies[e3];
        Flyer *testEnemy4 = new Flyer[e4];
        ...
        // Test loading multiple enemies instances
        ...
        collectable.ring.setIntRect(0, 0, 19, 19);
        ...
        while (window.isOpen())
        {
            ...
            for (int i = 0; i < e1; i++)
            {
                ...
                {
                    // Hide the enemies by moving it off-screen
                    testEnemy1[i].changePosition({-999.f, -999.f});
                    continue;
                }
            ...
            for (int i = 0; i < e2; i++)
            {
                ...
                // Check if the enemies is off-screen and move it out of view if so
                if (screenX + testEnemy2[i].getWidth() < 0 || screenX > screenResolution.x)
                {
                    testEnemy2[i].changePosition({-999.f, -999.f});
                    continue;
                }
                ...
                // Check if there's a block on the right side of the enemies
                int colRight = static_cast<int>((worldX + testEnemy2[i].getWidth()) / 50); // Right side of the enemies
                int rowBelow = static_cast<int>((worldY + hitbox.getHeight()) / 50);       // Tile just below the enemies

                // Check the tile to the right of the enemies
                ...
                // Check if there's a block on the left side of the enemies
                int colLeft = static_cast<int>((worldX) / 50); // Left side of the enemies
                ...
                // Check if Sonic and enemies are both on the screen
                if (sonic.getLeft() >= 0 && sonic.getRight() <= screenResolution.x &&
                    screenX >= 0 && screenX <= screenResolution.x)
                {
                    // Move enemies towards Sonic on the x-axis if no obstacle
                    float e_S = 70.0f;
                ...
                // Update the enemies position on screen
            ...
            for (int i = 0; i < e4; i++)
            {
                ...
                // Check if Sonic is below the enemies within horizontal range
            ...
            while (window.pollEvent(event))
            {
                ...
                if (event.type == Event::MouseButtonPressed)
                {
                    ...
                    if (event.mouseButton.button == Mouse::Left)
                    {
                        ...
                        if (Back.isClicked({mousePosition.x,mousePosition.y}))
                        {
                            // Back.ChangeColor();
            ...
            animateRings(ringsElapsedTime, animationSpeed, ringFrame, collectable.ring);
            ...
            for (int i = 0; i < 100; i++)
            {
                if (!fireBallsUnlimited[i].isDestroyed)
                {
                    ...
                    else
                    {
                        ...
                        fireBallsUnlimited[i].sprite.setPosition({screenPos.x,screenPos.y});
                    }
                }
            }
          ...
          // ---- Same logic for testEnemy2 ----
                ...
                for (int j = 0; j < cols; ++j)
                {
                    ...
                    case 'R': // Ring
                        if (!ringCollected[i][j])
                        {
                            ...
                            {
                                collectable.ring.changePosition(vector2f(px + 15, py + 20));
                                collectable.ring.draw(window);
                            }
                        }
                        break;
                    }
            ...
            {
                showLevelUpScreen(window, sonic, 3.5f); // 5 seconds level-up screen
        ...
        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < cols; j++)
            {
                ...
                if (tile == '1' && idx1 < e1)
                {
                    testEnemy1[idx1].changeImage("CrabMeat.png");
                    testEnemy1[idx1].changeScale({1.45f, 1.3f});
                    testEnemy1[idx1].changePosition(vector2f(position.x,position.y));
                    testEnemy1[idx1].frameCount = 8;
                    testEnemy1[idx1].frameTime = 0.15f;
                    testEnemy1[idx1].x = position.x;
                    testEnemy1[idx1].y = position.y;
                ...
                else if (tile == '2' && idx2 < e2)
                {
                    testEnemy2[idx2].changeImage("motoBug.png");
                    testEnemy2[idx2].changeScale({1.12f, 1.12f});
                    testEnemy2[idx2].changePosition(vector2f(position.x,position.y));
                    testEnemy2[idx2].frameCount = 4;
                    testEnemy2[idx2].frameTime = 0.15f;
                    testEnemy2[idx2].x = position.x;
                    testEnemy2[idx2].y = position.y;
                ...
                else if (tile == '3' && idx3 < e3)
                {
                    testEnemy3[idx3].changeImage("beeBot.png");
                    testEnemy3[idx3].changeScale({2.f, 2.f});
                    testEnemy3[idx3].changePosition(vector2f(position.x,position.y));
                    testEnemy3[idx3].frameCount = 5;
                    testEnemy3[idx3].frameTime = 0.15f;
                    testEnemy3[idx3].x = position.x;
                    testEnemy3[idx3].y = position.y;
                ...
                else if (tile == '4' && idx4 < e4)
                {
                    testEnemy4[idx4].changeImage("batBrain.png");
                    testEnemy4[idx4].changeScale({1.5f, 1.5f});
                    testEnemy4[idx4].changePosition(vector2f(position.x,position.y));
                    testEnemy4[idx4].frameCount = 8;
                    testEnemy4[idx4].frameTime = 0.15f;
                    testEnemy4[idx4].x = position.x;
                    testEnemy4[idx4].y = position.y;
    ...
    void levelTwo(RenderWindow &window, Event &event)
    {
        ...
        background.sprite.setColor(Color::Blue); // Icy Blue
        ...
        while (window.isOpen())
        {
            ...
            hpBar.setSize({200.f * (sonic.hp / 100.f), 20.f});
        ...
        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < cols; j++)
            {
                ...
                if (tile == '1' && idx1 < e1)
                {
                    testEnemy1[idx1].changeImage("CrabMeat.png");
                    testEnemy1[idx1].changeScale({1.45f, 1.3f});
                    testEnemy1[idx1].changePosition({position.x,position.y});
                    testEnemy1[idx1].frameCount = 8;
                    testEnemy1[idx1].frameTime = 0.15f;
                    testEnemy1[idx1].x = position.x;
                    testEnemy1[idx1].y = position.y;
                ...
                else if (tile == '2' && idx2 < e2)
                {
                    testEnemy2[idx2].changeImage("motoBug.png");
                    testEnemy2[idx2].changeScale({1.12f, 1.12f});
                    testEnemy2[idx2].changePosition({position.x,position.y});
                    testEnemy2[idx2].frameCount = 4;
                    testEnemy2[idx2].frameTime = 0.15f;
                    testEnemy2[idx2].x = position.x;
                    testEnemy2[idx2].y = position.y;
                ...
                else if (tile == '3' && idx3 < e3)
                {
                    testEnemy3[idx3].changeImage("beeBot.png");
                    testEnemy3[idx3].changeScale({2.f, 2.f});
                    testEnemy3[idx3].changePosition({position.x,position.y});
                    testEnemy3[idx3].frameCount = 5;
                    testEnemy3[idx3].frameTime = 0.15f;
                    testEnemy3[idx3].x = position.x;
                    testEnemy3[idx3].y = position.y;
                ...
                else if (tile == '4' && idx4 < e4)
                {
                    testEnemy4[idx4].changeImage("batBrain.png");
                    testEnemy4[idx4].changeScale({1.5f, 1.5f});
                    testEnemy4[idx4].changePosition({position.x,position.y});
                    testEnemy4[idx4].frameCount = 8;
                    testEnemy4[idx4].frameTime = 0.15f;
                    testEnemy4[idx4].x = position.x;
                    testEnemy4[idx4].y = position.y;
            ...
            while (window.pollEvent(event))
            {
                ...
                if (event.type == Event::MouseButtonPressed)
                {
                    ...
                    if (event.mouseButton.button == Mouse::Left)
                    {
                        ...
                        else if (level1.isClicked({mousePosition.x,mousePosition.y}))
                        {
                            cout << "Level 1 Selected" << endl;

                            levelOne(window, event);
                        }
                        else if (level2.isClicked({mousePosition.x,mousePosition.y}))
                        {
                            cout << "Level 2 Selected" << endl;
                            levelTwo(window, event);
                        }
                        else if (level3.isClicked({mousePosition.x,mousePosition.y}))
                        {
                            cout << "Level 3 Selected" << endl;
                            levelThree(window, event);
                            //  Add logic for Level 3
                        }
                        else if (bossLevel.isClicked({mousePosition.x,mousePosition.y}))
                        {
    ...
    void askingUsername(RenderWindow &window, Event &event, string &username, scoreBoard *ranking, int &ranks)
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == Event::MouseButtonPressed)
            {
                ...
                if (event.mouseButton.button == Mouse::Left)
                {
                    ...
                    else if (volumeSlow.isClicked({mousePosition.x,mousePosition.y}))
                    {
                    ...
                    else if (volumeMedium.isClicked({mousePosition.x,mousePosition.y}))
                    {
                    ...
                    else if (volumeFast.isClicked({mousePosition.x,mousePosition.y}))
                    {
    ...
    void menuWorking(RenderWindow &window, Event &event, Button &NewGame, Button &Options, Button &Continue, Button &LeaderBoard, Button &exitButton, Object &sonicLogo, Object &menuBackground, scoreBoard *&ranking)
    {
        // This While loop handle all the Inputs of ( Keyboard --- Mouse --- Screen Buttons )
        while (window.pollEvent(event))
        {
            ...
            if (event.type == Event::MouseButtonPressed)
            {
                ...
                if (exitButton.isClicked({mousePosition.x,mousePosition.y}))
                {
                    exit(EXIT_SUCCESS);
                }
                ...
                if (event.mouseButton.button == Mouse::Left)
                {
                    ...
                    if (NewGame.isClicked({mousePosition.x,mousePosition.y}))
                    {
                        // NewGame.ChangeColor();
                    ...
                    else if (Options.isClicked({mousePosition.x,mousePosition.y}))
                    {
                        // Options.ChangeColor();
                    ...
                    else if (Continue.isClicked({mousePosition.x,mousePosition.y}))
                    {
                    ...
                    else if (LeaderBoard.isClicked({mousePosition.x,mousePosition.y}))
                    {
    ...
    void menu_game()
    {
        ...
        LoadSound(backgroundMusic, "labrynth.ogg", true);
    ...
    void LeaderBoardWorkingScreen(RenderWindow &window, Event &event, Object &leaderBoardBackground, Button &Back, scoreBoard *ranking, int &ranks)
    {
        // This While loop handle all the Inputs of ( Keyboard --- Mouse --- Screen Buttons )
...
void fileRead(scoreBoard *&ranking, int &totalRanks)
{
    ...
    scoreBoard temp[100]; // Max 100 entries
    int totalInFile = 0;
    int pos, score;
    string name;
    ...
    while (file >> pos)
    {
        ...
        temp[totalInFile] = scoreBoard(pos, name, score);
        totalInFile++;
    }
    ...
    ranking = new scoreBoard[totalRanks];
