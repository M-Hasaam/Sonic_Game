...
Vector2f getSpriteSize(Sprite &sprite)
{
...
void levelOne(RenderWindow &window, Event &event)
{
    ...
    while (window.isOpen())
    {
        ...
        for (int i = 0; i < e1; i++)
        {
            ...
            Vector2f ePos = testEnemy1[i].sprite.getPosition();
            Vector2f eSize = getSpriteSize(testEnemy1[i].sprite);
            Vector2f sPos = hitbox.sprite.getPosition();
            Vector2f sSize = getSpriteSize(hitbox.sprite);
            Vector2f tPos = tales.sprite.getPosition();
            Vector2f tSize = getSpriteSize(tales.sprite);
            Vector2f kPos = knuckles.sprite.getPosition();
            Vector2f kSize = getSpriteSize(knuckles.sprite);

            bool collided = false;

            if (sPos.x < ePos.x + eSize.x && sPos.x + sSize.x > ePos.x &&
                sPos.y < ePos.y + eSize.y && sPos.y + sSize.y > ePos.y)
            {
                float sonicBottom = sPos.y + sSize.y;
                float enemyTop = ePos.y;
            ...
            if (tPos.x < ePos.x + eSize.x && tPos.x + tSize.x > ePos.x &&
                tPos.y < ePos.y + eSize.y && tPos.y + tSize.y > ePos.y)
            {
                float talesBottom = tPos.y + tSize.y;
                float enemyTop = ePos.y;
            ...
            if (kPos.x < ePos.x + eSize.x && kPos.x + kSize.x > ePos.x &&
                kPos.y < ePos.y + eSize.y && kPos.y + kSize.y > ePos.y)
            {
                float knucklesBottom = kPos.y + kSize.y;
                float enemyTop = ePos.y;
        ...
        for (int i = 0; i < e2; i++)
        {
            ...
            Vector2f ePos = testEnemy2[i].sprite.getPosition();
            Vector2f eSize = getSpriteSize(testEnemy2[i].sprite);
            Vector2f sPos = hitbox.sprite.getPosition();
            Vector2f sSize = getSpriteSize(hitbox.sprite);
            Vector2f tPos = tales.sprite.getPosition();
            Vector2f tSize = getSpriteSize(tales.sprite);
        ...
        for (int i = 0; i < e3; i++)
        {
            ...
            Vector2f ePos = testEnemy3[i].sprite.getPosition();
            Vector2f eSize = getSpriteSize(testEnemy3[i].sprite);
            Vector2f sPos = hitbox.sprite.getPosition();
            Vector2f sSize = getSpriteSize(hitbox.sprite);
            Vector2f tPos = tales.sprite.getPosition();
            Vector2f tSize = getSpriteSize(tales.sprite);
        ...
        for (int i = 0; i < e4; i++)
        {
            ...
            Vector2f ePos = testEnemy4[i].sprite.getPosition();
            Vector2f eSize = getSpriteSize(testEnemy4[i].sprite);
            Vector2f sPos = hitbox.sprite.getPosition();
            Vector2f sSize = getSpriteSize(hitbox.sprite);
            Vector2f tPos = tales.sprite.getPosition();
            Vector2f tSize = getSpriteSize(tales.sprite);
        ...
        for (int i = 0; i < rows; ++i)
        {
            for (int j = 0; j < cols; ++j)
            {
                ...
                case 'S':
                {
                    ...
                    Vector2f playerPos = hitbox.sprite.getPosition();
                    Vector2f playerSize = getSpriteSize(hitbox.sprite);

                    float playerLeft = playerPos.x;
                    float playerTop = playerPos.y;
                    float playerRight = playerLeft + playerSize.x;
                    float playerBottom = playerTop + playerSize.y;
                ...
                case 'R': // Ring
                    if (!ringCollected[i][j])
                    {
                        ...
                        if (playerPos.x < ringX + ringW && playerPos.x + playerSize.x > ringX &&
                            playerPos.y < ringY + ringH && playerPos.y + playerSize.y > ringY)
                        {
                            ringSound.play();
                            ringCollected[i][j] = true;
                            sonic.addRings();
                        }
                        else
                        {
                            ring1.changePosition(Vector2f(ringX, ringY));
                            ring1.draw(window);
                        }
                    }
...
void levelTwo(RenderWindow &window, Event &event)
{
    ...
    while (window.isOpen())
    {
        ...
        for (int i = 0; i < e1; i++)
        {
            ...
            Vector2f enemyPos = testEnemy1[i].sprite.getPosition();
            float enemyLeft = enemyPos.x;
            float enemyTop = enemyPos.y;
            float enemyRight = enemyLeft + testEnemy1[i].sprite.getGlobalBounds().width;
            float enemyBottom = enemyTop + testEnemy1[i].sprite.getGlobalBounds().height;

            Vector2f sonicPos = hitbox.sprite.getPosition();
            float sonicLeft = sonicPos.x;
            float sonicTop = sonicPos.y;
            float sonicRight = sonicLeft + hitbox.sprite.getGlobalBounds().width;
            float sonicBottom = sonicTop + hitbox.sprite.getGlobalBounds().height;

            if (sonicRight > enemyLeft && sonicLeft < enemyRight &&
                sonicBottom > enemyTop && sonicTop < enemyBottom)
            {
                if (sonicBottom - enemyTop <= 10.0f)
                {
                    cout << "Sonic killed enemy1 from above\n";
            ...
            Vector2f talesPos = tales.sprite.getPosition();
            float talesLeft = talesPos.x;
            float talesTop = talesPos.y;
            float talesRight = talesLeft + tales.sprite.getGlobalBounds().width;
            float talesBottom = talesTop + tales.sprite.getGlobalBounds().height;

            if (talesRight > enemyLeft && talesLeft < enemyRight &&
                talesBottom > enemyTop && talesTop < enemyBottom)
            {
                if (talesBottom - enemyTop <= 10.0f)
                {
                    cout << "Tales killed enemy1 from above\n";
            ...
            Vector2f knucklesPos = knuckles.sprite.getPosition();
            float knucklesLeft = knucklesPos.x;
            float knucklesTop = knucklesPos.y;
            float knucklesRight = knucklesLeft + knuckles.sprite.getGlobalBounds().width;
            float knucklesBottom = knucklesTop + knuckles.sprite.getGlobalBounds().height;

            if (knucklesRight > enemyLeft && knucklesLeft < enemyRight &&
                knucklesBottom > enemyTop && knucklesTop < enemyBottom)
            {
                if (knucklesBottom - enemyTop <= 10.0f)
                {
                    cout << "Knuckles killed enemy1 from above\n";
        ...
        for (int i = 0; i < e2; i++)
        {
            ...
            Vector2f enemyPos = testEnemy2[i].sprite.getPosition();
            float enemyLeft = enemyPos.x;
            float enemyTop = enemyPos.y;
            float enemyRight = enemyLeft + testEnemy2[i].sprite.getGlobalBounds().width;
            float enemyBottom = enemyTop + testEnemy2[i].sprite.getGlobalBounds().height;
        ...
        for (int i = 0; i < e3; i++)
        {
            ...
            Vector2f enemyPos = testEnemy3[i].sprite.getPosition();
            float enemyLeft = enemyPos.x;
            float enemyTop = enemyPos.y;
            float enemyRight = enemyLeft + testEnemy3[i].sprite.getGlobalBounds().width;
            float enemyBottom = enemyTop + testEnemy3[i].sprite.getGlobalBounds().height;
        ...
        for (int i = 0; i < e4; i++)
        {
            ...
            Vector2f enemyPos = testEnemy4[i].sprite.getPosition();
            float enemyLeft = enemyPos.x;
            float enemyTop = enemyPos.y;
            float enemyRight = enemyLeft + testEnemy4[i].sprite.getGlobalBounds().width;
            float enemyBottom = enemyTop + testEnemy4[i].sprite.getGlobalBounds().height;
        ...
        for (int i = 0; i < rows; ++i)
        {
            for (int j = 0; j < cols; ++j)
            {
                ...
                case 'S':
                {
                    ...
                    float playerLeft = playerPos.x;
                    float playerTop = playerPos.y;
                    float playerRight = playerLeft + hitbox.sprite.getGlobalBounds().width;
                    float playerBottom = playerTop + hitbox.sprite.getGlobalBounds().height;

                    if (spikeRight > playerLeft && spikeLeft < playerRight &&
                        spikeBottom > playerTop && spikeTop < playerBottom && !isImmune)
                    {
                ...
                case 'R': // Ring
                {
                    if (!ringCollected[i][j])
                    {
                        ...
                        Vector2f ringPos(px + 15, py + 20); // Ring position
                        Vector2f playerPos = hitbox.sprite.getPosition();
                        float playerLeft = playerPos.x;
                        float playerTop = playerPos.y;
                        float playerRight = playerLeft + hitbox.sprite.getGlobalBounds().width;
                        ...
                        // Define ring bounds
                        float ringLeft = ringPos.x;
                        float ringTop = ringPos.y;
                        float ringRight = ringLeft + 19; // Ring width
                        float ringBottom = ringTop + 19; // Ring height

                        // Check for intersection between player and ring
                        if (ringRight > playerLeft && ringLeft < playerRight &&
                            ringBottom > playerTop && ringTop < playerBottom)
                        {
                            // Play the ring sound
                            ringSound.play();
...
void levelThree(RenderWindow &window, Event &event)
{
    ...
    while (window.isOpen())
    {
        ...
        for (int i = 0; i < e1; i++)
        {
            ...
            if (enemyRight > sonicLeft && enemyLeft < sonicRight &&
                enemyBottom > sonicTop && enemyTop < sonicBottom)
            {
                if (sonicBottom - enemyTop <= 10.0f)
                {
                    cout << "Sonic killed enemy1 from above\n";
                ...
                else
                {
                    if (!isImmune)
                    {
                        cout << "Knuckles hit enemy1 from side -> Sonic loses 1 HP\n";
                        sonic.takeDamage(5);
                    }
            ...
            // Check collision with Tales
            if (enemyRight > talesLeft && enemyLeft < talesRight &&
                enemyBottom > talesTop && enemyTop < talesBottom)
            {
                if (talesBottom - enemyTop <= 10.0f)
                {
                    cout << "Tales killed enemy1 from above\n";
            ...
            // Check collision with Knuckles
            if (enemyRight > knucklesLeft && enemyLeft < knucklesRight &&
                enemyBottom > knucklesTop && enemyTop < knucklesBottom)
            {
                if (knucklesBottom - enemyTop <= 10.0f)
                {
                    cout << "Knuckles killed enemy1 from above\n";
        ...
        for (int i = 0; i < e2; i++)
        {
                ...
                enemyBottom > sonicTop && enemyTop < sonicBottom)
            {
                ...
                else
                {
                    if (!isImmune)
                    {
                        cout << "Knuckles hit enemy2 from side -> Sonic loses 1 HP\n";
                        sonic.takeDamage(5);
                    }
        ...
        for (int i = 0; i < e4; i++)
        {
                ...
                enemyBottom > sonicTop && enemyTop < sonicBottom)
            {
                ...
                else
                {
                    if (!isImmune)
                    {
                        cout << "Sonic hit enemy4 from side -> Sonic loses 1 HP\n";
                        sonic.takeDamage(5);
                    }
        ...
        for (int i = 0; i < rows; ++i)
        {
            for (int j = 0; j < cols; ++j)
            {
                ...
                case 'R': // Ring
                    ...
                    {
                        // Same as before, handle ring collection logic here
                        float ringLeft = px + 15;
                        float ringTop = py + 20;
                        float ringRight = ringLeft + 19;
                        float ringBottom = ringTop + 19;
